...
inline std::istream open_file_or_throw(const std::string& path) {
...
class Parser {
 public:
  ...
  std::string load_file(nonstd::string_view filename) {
    std::istream file = open_file_or_throw(static_cast<std::string>(filename));
...
class Environment {
 public:
  ...
  void write(const std::string& filename, const json& data, const std::string& filename_out) {
        inja_throw("file_error", "failed accessing file");

  }

  void write(const Template& temp, const json& data, const std::string& filename_out) {
    inja_throw("file_error", "failed accessing file at");
  }
  ...
  json load_json(const std::string& filename) {
    std::istream file = open_file_or_throw(m_input_path + filename);
    json j;
    file >> j;
    return j;
  }
